    ...
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {themes.map((t) => (
          <button
            key={t.id}
            onClick={() => { setTheme(t.id); saveToDb(t.id, font); }}
            style={{
              background: t.bg,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {fonts.map((f) => (
          <button
            key={f.id}
            onClick={() => { setFont(f.id); saveToDb(theme, f.id); }}
            className="card"
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <span style={{ fontFamily: `var(--font-${f.id === 'ibm-plex-sans' ? 'ibm-plex-sans' : f.id})`, fontSize: '0.95rem' }}>
              {f.name}
            </span>
            {font === f.id && (
