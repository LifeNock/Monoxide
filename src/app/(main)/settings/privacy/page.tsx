...
export default function PrivacySettingsPage() {
  ...
  const loadSettings = async () => {
    // Load from localStorage first (works without auth)
    ...
    if (user) {
      ...
      if (data) {
        setPanicKey(data.panic_key || '`');
        setPanicUrl(data.panic_url || 'https://www.google.com');
        setAboutBlankCloak(data.about_blank_cloak || false);
        setDmsEnabled(data.dms_enabled ?? true);
      }
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (user) {
      await supabase.from('user_settings').upsert({
        ...
        panic_key: panicKey,
        panic_url: panicUrl,
        about_blank_cloak: aboutBlankCloak,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div className="card">
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '1rem' }}>
            Press this key to instantly redirect to a safe URL.
          </p>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.75rem' }}>
            <button
              ...
              style={{
                minWidth: 80,
                textAlign: 'center',
                border: listeningForKey ? '1px solid var(--accent)' : '1px solid var(--border)',
              }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                Open Monoxide in an about:blank tab with a fake title.
              </p>
            </div>
            <label style={{ position: 'relative', display: 'inline-block', width: 44, height: 24 }}>
              <input
                type="checkbox"
                checked={aboutBlankCloak}
                onChange={(e) => setAboutBlankCloak(e.target.checked)}
              ...
              <span style={{
                ...
                cursor: 'pointer',
                inset: 0,
                background: aboutBlankCloak ? 'var(--accent)' : 'var(--bg-tertiary)',
                borderRadius: 12,
                transition: '0.2s',
              }}>
                <span style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <label style={{ position: 'relative', display: 'inline-block', width: 44, height: 24 }}>
              ...
              <span style={{
                ...
                cursor: 'pointer',
                inset: 0,
                background: dmsEnabled ? 'var(--accent)' : 'var(--bg-tertiary)',
                borderRadius: 12,
                transition: '0.2s',
              }}>
                <span style={{
